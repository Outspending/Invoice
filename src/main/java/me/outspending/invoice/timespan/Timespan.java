...
public class Timespan {
    ...
    public static Timespan createTimespan(long seconds) {
        long interval = System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(seconds);
        return new Timespan(interval);
    }

    public static Timespan createTimespan(long seconds, long minutes) {
        long interval = System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(seconds) +
                TimeUnit.MINUTES.toMillis(minutes);
        return new Timespan(interval);
    }

    public static Timespan createTimespan(long seconds, long minutes, long hours) {
        long interval = System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(seconds) +
                TimeUnit.MINUTES.toMillis(minutes) + TimeUnit.HOURS.toMillis(hours);
        return new Timespan(interval);
    }

    public static Timespan createTimespan(long seconds, long minutes, long hours, long days) {
        long interval = System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(seconds) +
                TimeUnit.MINUTES.toMillis(minutes) + TimeUnit.HOURS.toMillis(hours) +
